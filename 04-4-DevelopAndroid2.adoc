...
. The corresponding complete view definition in the `content_main.xml` file is the following. We are using <RelativeLayout> now:
+
[source,xml]
----
...
. The complete string definitions go in the `res/values/strings.xml` resource
+
[source,xml]
----
<resources>
...
. See https://github.com/Rijul5/eventregistration3/tree/android for the complete solution of mobile frontend.
